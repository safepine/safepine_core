...
public:
  ...
  { 
    ...
    for(int i = 0; i<data_frame.length; i++)
    {
      ...
      result~="0.0"~","; // Adjusted open n.a.
      result~="0.0"~","; // Adjusted high n.a.
      result~="0.0"~","; // Adjusted low n.a.
      result~=to!string(data_frame[i].price.adjclose)~","; 
      result~="0.0"; // Adjusted volume n.a.
      result~="\n";
    }   
    return result;
  }
  ...
  // Mine template
  void Mine(logger log = logger.on)(
    Date begin, 
    Date end, 
    string name, 
    intervals interval=intervals.daily,
    ...
    string cachePath = "cache/")
  {
    ...
    try
    {
      PublicAccessPoint accessPoint;
      accessPoint.profile[0] = "https://query1.finance.yahoo.com/v7/finance/download/";
      accessPoint.profile[1] =  "?period1=";
      accessPoint.profile[2] = "&period2=";
      accessPoint.profile[3] = "&interval=";
      accessPoint.profile[4] = "&events=history&includeAdjustedClose=true";

      // Assemble query. Use unix time.
      _query = accessPoint.profile[0]~_name~accessPoint.profile[1]~_beginUnix_s~accessPoint.profile[2]~_endUnix_s~accessPoint.profile[3]~interval~accessPoint.profile[4];
      string shadow_content = to!string( get(_query));
      auto prices = shadow_content.csvReader!Price_csvReader(',');

      accessPoint.profile[4] = "&events=splits&includeAdjustedClose=true";
      _query = accessPoint.profile[0]~_name~accessPoint.profile[1]~_beginUnix_s~accessPoint.profile[2]~_endUnix_s~accessPoint.profile[3]~interval~accessPoint.profile[4];
      shadow_content = to!string( get(_query));
      auto splits = shadow_content.csvReader!Split_csvReader(',');

      accessPoint.profile[4] = "&events=divs&includeAdjustedClose=true";
      _query = accessPoint.profile[0]~_name~accessPoint.profile[1]~_beginUnix_s~accessPoint.profile[2]~_endUnix_s~accessPoint.profile[3]~interval~accessPoint.profile[4];
      shadow_content = to!string( get(_query));
...
private:
  ...
  string _beginUnix_s; // begin date in unix time. needed to bind
  string _endUnix_s;  // end date in unix time. needed to bind
