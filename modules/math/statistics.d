...
struct Swings {
  double[] swing_high_values, swing_low_values;
  string[] swing_high_dates, swing_low_dates;
}

Swings compute_swings(
  double[] prices_IN, 
  string[] dates_IN, 
  double swingPercentage_IN) 
{
  Swings result;
  double swingPercentage = swingPercentage_IN, pivot_1, pivot_2;
  string pivot_1_date, pivot_2_date;
  pivot_1 = prices_IN[0];
  pivot_2 = pivot_1; 
  pivot_1_date = dates_IN[0];
  pivot_2_date = pivot_1_date;
  int swingDirection = 1, swingDirection_d = 1; 
  for(int j = 1; j < dates_IN.length; ++j) {
    swingDirection_d = swingDirection;

    if(prices_IN[j] - prices_IN[j-1] >= 0) {
      pivot_1 = prices_IN[j];
      pivot_1_date = dates_IN[j];
      swingDirection = 1;
    }

    if(prices_IN[j] - prices_IN[j-1] < 0) {
      pivot_2 = prices_IN[j];
      pivot_2_date = dates_IN[j];
      swingDirection = -1;
    }

    if(swingDirection_d == 1 && swingDirection == -1) {
      if(result.swing_high_values.length == 0) {
        result.swing_high_values ~= pivot_1;
        result.swing_high_dates ~= pivot_1_date;            
      }
      else if( pivot_1 > result.swing_high_values[result.swing_high_values.length-1]*(1 + swingPercentage) || pivot_1 < result.swing_high_values[result.swing_high_values.length-1]*(1 - swingPercentage)) {
        result.swing_high_values ~= pivot_1;
        result.swing_high_dates ~= pivot_1_date;
      }
    }
    if(swingDirection_d == -1 && swingDirection == 1) {
      if(result.swing_low_values.length == 0) {
        result.swing_low_values ~= pivot_1;
        result.swing_low_dates ~= pivot_1_date;
      }
      else if( pivot_2 > result.swing_low_values[result.swing_low_values.length-1]*(1 + swingPercentage)  || pivot_2 < result.swing_low_values[result.swing_low_values.length-1]*(1 - swingPercentage)) {
        result.swing_low_values ~= pivot_2;
        result.swing_low_dates ~= pivot_2_date;
      }
    }
  }
